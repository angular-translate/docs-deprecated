NG_DOCS={
  ...
  "pages": [
    ...
    },
    {
      ...
      "keywords": "$emit $on $rootscope $scope $translate $translatechangesuccess $translateprovider accessed achieve add angular angular-translate anotherone app application approach apps asynchronous asynchronously aware awesome basic behaves bind binding building call callback case cases change changed changing code config content contents controller controllers convenience ctrl data deal default dependency determine directive directly display documents doesn don easy en error exist expose extend fail failed features fired function general going guide hard headline html ids imagine info inject injection inside installed json language languages level library ll loading logic love manipulate mind module multiple myapp namespace namespaced namespaced_paragraph needed ng-app ng-controller object overview paragraph pascalprecht pass performance post preferredlanguage promise properties provide provided re-execute reasons registered rejected request requesting resolved result return returns running runtime sample service services simply srsly started successful table tag things time title translate translated translating translation translationid translationids translations turned two-way unicorn updated usage var version versioninfo ways wonder working works world wouldn wrapping"
    },
    {
      "section": "guide",
    ...
    },
    {
      ...
      "keywords": "$compile $scope $translateprovider add addition alt angular angular-translate app attribute attribute_name attributes attrtranslations awesome beginners behavior benefits best better binding case choose compiling concrete config contents context controller controller-provided cool couldn covered create ctrl current dataset deprecated directive disable dynamically element en enable enabled example expected expressions false feature filter filters flexible function general globally great guide ha handle headline hey html identifier ids interpolate interpolated isn iterator layer logo looping love mentioned module myapp mylogo ng-controller object offers one-time overview paragraph parsed pascalprecht pass passed passed_as_attribute passed_as_interpolation passed_as_text png post preferredlanguage prefixing processed reach represents scope sets src srsly starting statement substitute supply table text texts title ton translate translate-attr translate-attr-attribute_name translate-compile translation translation_id translations turned unicorn unnecessary update updated usage usepostcompiling var variable version view watch watched watches ways work works wouldn yeah"
    },
    {
      "section": "guide",
    ...
    },
    {
      ...
      "keywords": "$translateprovider accessing actual add angular-translate app arguments array automatic automatically browser called case configuration conjunction cover covered de de_ de_ch de_de declare detection determine determinepreferredlanguage discussed en en_ en_uk en_us error expects feature fetched files glance guide key language languages lead learned love map mapped maps mention method multi-language_determining-preferred-language-automatically negotiation object overview parameter preferred properties recognise registeravailablelanguagekeys relation relations returned returns second sense set smart support supports system table tables team territory translation translations treat unicorn variation wildcards"
    },
    {
      "section": "guide",
      "id": "10_storages",
    ...
    },
    {
      ...
      "keywords": "$interpolate $locale $scope $translate $translateprovider add addinterpolation addition alright angular angular-translate app appends basic benutze bin build building button_lang_de button_lang_en changelanguage chapters config context controller cool ctrl current currently custom custominterpolation de default deutsch eine en englisch english es exactly expects explicitly factory familiar fand fanden features female follow friend function german getinterpolationidentifier guide gut headline ich identifier implement implementing implements integrate interface interpolate interpolateparams interpolates interpolation interpolations interpolators language learned library loaders locale logic loose love male mapped messageformat method methods module myapp ng-click ng-controller object optional override overridinginterpolationtemporarily overview params pascalprecht plug plural preferredlanguage prepends pretty previous provided read remember return returns runtime sanitizestrategy select service services setlocale sets simply smaller smart specific start storages store string strings structure text things translate translate-values translationid translations type unicorn useinterpolation useloader usestorage val var ve work"
    },
    {
      "section": "guide",
      ...
      "keywords": "$scope $translate $translateprovider advantage also_not_existing angular angular-translate angular-translate-handler-log app application belongs benutze bin bower browser button_lang_de button_lang_en call case cccccc changelanguage color complex config configuring console controller ctrl de debug default deutsch doesn don eine en enable enables englisch english error es exist extension extensions fand fanden feature female function functionality german guide gut handler handlers handling headline ich install interpolatedtranslation interpolation key lang log logged logs love male missing module myapp ng-click ng-controller not_existing overview parameters params pascalprecht plural post postprocess preferredlanguage processing rest return sample screen select special specific style team text translate translation translationid translations translators unicorn usemissingtranslationhandlerlog val valuable var version warnings working wrong"
    },
    {
      "section": "guide",
    ...
    },
    {
      ...
      "keywords": "actual add additional angular-translate app apps boolean capable case cleanprevstrings compatible configuration convert converting covers create created creates creating currently deeper default depends destination dev developing development devonefile dist enable entire environment example extracts false file files format formatted generate generated generation gettext github grunt grunt-angular-translate grunt-po2json-angular-translate gruntfile guide handy head help html ids initconfig install interested io isn javascript js json json-export loader loadnpmtasks maintaining managing medium module msgid multiple names ngtranslateeditor npm options optionsenablealtplaceholders optionsuppercaseid output overview placeholders pluralization po po-formatted po2json_angular_translate pretty previous print project read reads remove repo result set setting single sized small special standard steps strings tables task test text things tmp tool tools translation translations true type universe uppercase uppercaseid wanna webpack webpack-angular-translate writes your_target"
    },
    {
      "section": "guide",
