NG_DOCS={
  ...
  "pages": [
    ...
    },
    {
      ...
      "keywords": "abh alle alles als andernfalls angesehen angular-translate anschlie au auch auf aus ausf ausgef aussehen auszuf befehl befehle befinden benutzt bietet bitte bower bower-package build builden building clone conf damit danke darauf das dass datei dateien davon dein deiner der die diese dir direkt dist du ein einbinden eine einem einen einer einfach enth entsprechenden entwicklung entwicklungs- entwicklungszwecke erfolgreich erstellten es folgende folgenden framework funktioniert ganz geklonte gelieferten generieren generiert gibt git github gr grunt gruntfile guide hast history html-dokument inklusive ins install installation installiere installieren installiert io ist jemals jetzt js kann kannst karma klon klonen kofigurierte kommandozeile kommen kommt komplette kompletten komponentenverzeichnis konfigurationsdatei konfiguriert liegt loslegen love man maschine midway min mit mitgelieferte nachdem navigiere npm nur overview package passiert prodkuktionsumgebungen productionsversion produktion projekt pull quelltext reduziert repository request richtig sehen selbstverst sende sich sicherstellen sie sind sollten solltest source sowohl spiel start starte startet tasks terminal testing testrunner testrunnter tests testsuite tools tun um und ungef unicorn unit verwendet verzeichnis vollst von wartet weg weitere weiterer weitergehen wenn werden wichtigste wie wird wo wurden zu"
    },
    {
      "section": "guide",
    ...
    },
    {
      ...
      "keywords": "$translateprovider alle allem als angenommen angular-translate anhand anwendung applikation argumente array auch auf aufgerufen aus auszudr automatischen beispiel beispiels ben bevorzugte beziehung beziehungen bietet blick browser browser-spracherkennung browserparameter bzw da dadurch daher damit das dass dazu de de_ de_ch de_de definiert deiner dem den der des determinepreferredlanguage die dies diese diesen dieser dieses durch eben eigentlich ein eine einem en en_ en_uk en_us ende ergibt erkennen erl ermittelt ersten erw erwartet es explizit falls fehler funktion funktionalit gehen gibt gleichen guide haben hervorgeht hier hinzugef intelligente ist kann kapitel kombination konfigurierten kurz lage language love man mappings mit mittels modul multi-language_automatische-wahl-der-praeferierten-sprache muss nachfolgenden negotiation nicht normalerweise nun nur nutzung objekt oder ohne overview parameter pr problematisch registeravailablelanguagekeys sachverhalt schl sein separat setzen sicher sinn sinnvoll sobald sprachauswahl sprache sprachen sprachschl sprachtabelle statt stehen stelle suchen team teilausdr translations und unicorn unter unterschiedliche unterst verweisen vom von vor wahl weitere weiterer wenn werden wie wildcards wir wird wollen wurden zeigen zu zueinander zum zur zus zusammenh zuweisung zweite zwischen"
    },
    {
      "section": "guide",
      "id": "10_storages",
    ...
    },
    {
      ...
      "keywords": "$scope $translate $translateprovider aktiviert alle als also_not_existing angular angular-translate angular-translate-handler-log app auch auf aufgerufen aufzurufen beispiel benutze bereit bestimmte bin bower browsers button_lang_de button_lang_en cccccc changelanguage color config controller ctrl da damit dass de debug-informationen default dem den der des deutsch die diese dieses dir ein eine einfach einfache en englisch english erfolg erhalten erm error es extension extensions falsche fand fanden fehlende female function funktion funktionalit gar gef gefunden genau german gibt guide gut handler handling hat headline hier ich identifikation inhalten ins install installieren interpolatedtranslation interpolation ist kann keine kommt komplex konsole lang log-handler-extension loggt love machen male man mehr methode missingtranslationhandler mit module modulkonfiguration muss myapp nach ng-click ng-controller nich nicht nichts not_existing nur nutzen nutzung ob oder overview parameter params pascalprecht plural post postprocess preferredlanguage processing rest return schau schl schnell seit select sich sicher sie spiel stellt style team text tmal translate translation translationid translations um unicorn usemissingtranslationhandler usemissingtranslationhandlerlog val var version versucht verwenden von vorkommen warnings wei weiter wenn werden wie wird zu zum zus"
    },
    {
      "section": "guide",
    ...
    },
    {
      ...
      "keywords": "$sanitize $scope $translateprovider aber aktivieren aktivierte allerdings allgemeine allgemeinen angular angular-translate angularjs anwendungsbeispiele app applikation auch ausgabe ausgabeform ausgaben ausgegeben behoben bei benutzung bereinigt bereits bis color config controller ctrl daf daher dann dar das dass dazu deaktiviert defekt den der derzeit die dies doppelt dringlich durch dynamische dynamischen dynamischer eigenen eine einen einer empfehlen empfehlung en enable encodiert entfernt entities ergebnis ernsthaften es escape escaped escapen escapeparameters escaping explizit feature folgende function gesamte gesamten gew gibt gilt grunds guide headline hello href html https inhalte innerhalb intern interpolation ist javascript kann konfiguriert korrekt lange love man mit mittels module myapp_escape_params myapp_escaped myapp_not_escaped myapp_sanitize ng-controller ngsanitize nicht nichts noch null nur org overview owasp paragraph parameter parametern pascalprecht php preferredlanguage regul sanitize sanitizeparameters sanitizing security-strategy sein selbstverst sichere sicherheit sicherheitsl solange sorgt sp standard standardstrategien stehen strategie style synonym translate translate-values translations um und unicorn unsicherer usesanitizevaluestrategy utf-8 var verf version verwenden verwendung vollst von warnhinweis weder welche werden werte wir wird wurde xss zeigen zu zur"
    },
    {
      "section": "guide",
      "id": "20_tools",
      ...
      "keywords": "aber aktiviere alle aller als andere angegeben angular-translate angular-translate-kompatibles anwendungsfall app arbeit auf aus ausgabe ausgabedateien beiden beispiel benutzt bestehenden bevor bitte boolean bzw cleanprevstrings create das dateien default default-text deinem dem den der derzeit dev devonefile die diesem dieses dingen dist ein eine eines einflu einstellungen einzelne ende entwicklungs- erstellt erzeugt es existieren export extrahieren false file files folgende format funktionalit gebracht gem generate gerne gettext-formatierungen gibt github gro grunt grunt-angular-translate grunt-po2json-angular-translate gruntfile guide haben handliche haus herum hier hilfsprogramme hinzu html-dateien individuelle informationen initconfig install installiere io ist javascript- je jeweils js json json-dateien kann kernmodul kleine kleines komplettes konfiguration konvertieren konvertierung lesbarere liest loadnpmtasks man mehr mehrere mit mittelgro module msgid nach names neue ngtranslateeditor nicht noch npm nur nutzen oder optionen options optionsenablealtplaceholders optionsuppercaseid overview passend platzhalter pluralization po po-foramtierte po2json_angular_translate praxisbeispiel pretty produktionsumgebungen projekt repo schau schreibt schritte sehr sein single sinnvolles standard-loader strings task team test tmp tollen tools translationid true type um und unterscheidung uppercaseid verzeichnis vom von webpack-angular-translate webpack-loader weitere weiteren welche werden werkzeug wichtige wichtigsten wird your_target zahlreiche zeichenketten ziel zieldateien zu zum zur zus"
    },
    {
      "section": "guide",
